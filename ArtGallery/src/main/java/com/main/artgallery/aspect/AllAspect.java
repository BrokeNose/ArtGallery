package com.main.artgallery.aspect;
import javax.servlet.http.HttpServletRequest;

import org.aspectj.lang.ProceedingJoinPoint;
import org.aspectj.lang.annotation.Around;
import org.aspectj.lang.annotation.Aspect;
//import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

//import com.main.artgallery.service.OpinionService;

@Aspect
@Component
public class AllAspect {
	
	//@Autowired
	//OpinionService oService;
	
	@Around("execution(public * *(..))")
	public Object headerSet(ProceedingJoinPoint joinPoint) throws Throwable {
		//AOP 가 적용된 메소드에 전달된 인자 얻어오기
		Object[] args=joinPoint.getArgs();
		//반복문 돌면서 하나씩 추출해서
		for(Object tmp:args){
			//만약 객체가 HttpServletRequest type 이면
			if(tmp instanceof HttpServletRequest) {
				//원래 type 으로 casting 해서
				HttpServletRequest request=(HttpServletRequest)tmp;
		
				//의견 갯수 -  headerOpinionCount
				//oService.getCountNoView(request);
				
				//query 문자열 읽어오기
				//a=xxx&b=xxx&c=xxx
				String query=request.getQueryString();
				if ( query.indexOf("url=", 0) < 0  ) {	// -1 없음
					//원래 가야할 요청명
					String url=null;
					if(query==null) {
						url=request.getRequestURI();
					} else {
						url=request.getRequestURI()+"?"+query;
					}
					request.setAttribute("headerUrl", url);
				}
				break;
			}
		}
		
		//정상적으로 수행하기
		return joinPoint.proceed();
	}
}